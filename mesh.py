...
def meshToCSS(mesh):
    rules = []
    for color in mesh:
        rules.append('radial-gradient(circle at '+str(color['percentx'])+'% '+str(color['percenty'])+'%, '+colorToHex(color)+', transparent 80%)')
    grad = ','.join(rules)
...
def setMeshColors(url):
    global currentMesh
    for color, newcolor in zip(currentMesh, getBestColors(url, 8)):
...
def getMeshColors(url):
    tempMesh = []
    for newcolor in getBestColors(url, 8):
        tempMesh.append({
            'red':   newcolor[0],
            'green': newcolor[1],
            'blue':  newcolor[2]
